...
class OAuth2AuthenticationSuccessHandlerTest {
    ...
    void usuarioNuevo_seCreaConRolScrumMember() throws Exception {
        when(appUserRepository.findByEmail("nuevo@mpdia.com")).thenReturn(Optional.empty());

        AppUser saved = new AppUser();
        saved.setId(UUID.randomUUID());
        saved.setEmail("nuevo@mpdia.com");
        ...
        Authentication auth = new TestingAuthenticationToken(googleUser("nuevo@mpdia.com", "Nuevo"), null);
        ...
        assertThat(jwtUtil.getEmail(token)).isEqualTo("nuevo@mpdia.com");
    ...
    void usuarioExistente_conservaRolYNoDuplica() throws Exception {
        ...
        existente.setEmail("existente@mpdia.com");
        existente.setRole("scrum_master");
        existente.setNombre("Nombre Original En BD");
        when(appUserRepository.findByEmail("existente@mpdia.com")).thenReturn(Optional.of(existente));

        MockHttpServletRequest request = new MockHttpServletRequest();
        MockHttpServletResponse response = new MockHttpServletResponse();
        // Google reporta un nombre distinto ("Nombre Distinto En Google") — no debe
        // pisar el que ya está guardado en BD.
        Authentication auth = new TestingAuthenticationToken(googleUser("existente@mpdia.com", "Nombre Distinto En Google"), null);
        ...
        assertThat(jwtUtil.getEmail(token)).isEqualTo("existente@mpdia.com");
    ...
    void usuarioCreadoPorPassword_alUsarGoogleConservaSuCuenta() throws Exception {
        AppUser existente = new AppUser();
        existente.setId(UUID.randomUUID());
        existente.setEmail("mixto@mpdia.com");
        existente.setPasswordHash("hash_bcrypt_real");
        existente.setRole("scrum_master");
        when(appUserRepository.findByEmail("mixto@mpdia.com")).thenReturn(Optional.of(existente));

        MockHttpServletRequest request = new MockHttpServletRequest();
        MockHttpServletResponse response = new MockHttpServletResponse();
        Authentication auth = new TestingAuthenticationToken(googleUser("mixto@mpdia.com", "Mixto"), null);
    ...
    void errorEnProcesamiento_redirigeConErrorControlado() throws Exception {
        ...
        MockHttpServletRequest request = new MockHttpServletRequest();
        MockHttpServletResponse response = new MockHttpServletResponse();
        Authentication auth = new TestingAuthenticationToken(googleUser("falla@mpdia.com", "Falla"), null);
