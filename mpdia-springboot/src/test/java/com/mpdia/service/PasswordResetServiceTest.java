...
class PasswordResetServiceTest {
    ...
    void setUp() {
        ...
        usuario.setEmail("usuario@mpdia.com");
    ...
    void solicitarRecuperacion_emailExistente_generaTokenYEnviaCorreo() {
        when(userRepo.findByEmail("usuario@mpdia.com")).thenReturn(Optional.of(usuario));
        when(emailService.enviar(anyString(), anyString(), anyString())).thenReturn(true);

        service.solicitarRecuperacion("usuario@mpdia.com");
        ...
        verify(emailService).enviar(eq("usuario@mpdia.com"), anyString(), bodyCaptor.capture());
    ...
    void solicitarRecuperacion_emailInexistente_noHaceNadaNiLanzaExcepcion() {
        when(userRepo.findByEmail("noexiste@mpdia.com")).thenReturn(Optional.empty());

        assertThatCode(() -> service.solicitarRecuperacion("noexiste@mpdia.com")).doesNotThrowAnyException();
